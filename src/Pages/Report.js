
// import React, { useState, useEffect } from 'react';
// import jsPDF from 'jspdf';
// import 'jspdf-autotable';
// import * as XLSX from 'xlsx';
// import { saveAs } from 'file-saver';
// import 'bootstrap/dist/css/bootstrap.min.css';
// import jsonData from './data.json'; // Import the data.json file

// const Report = () => {
//   const [data, setData] = useState([]); // Displayed data
//   const [originalData, setOriginalData] = useState([]); // Original unfiltered data
//   const [columns, setColumns] = useState([]);
//   const role = "Admin"; // Example role, replace with actual role from your auth logic

//   useEffect(() => {
//     setColumns(jsonData.columns);
//     setData(jsonData.data);
//     setOriginalData(jsonData.data);
//   }, []);

  
//   const downloadPdf = () => {
//     const doc = new jsPDF('l', 'pt', 'a4'); // Use landscape orientation for wider columns
//     const tableColumn = columns.map(col => col.Header); // Extract column headers
//     const tableRows = data.map(row => columns.map(col => row[col.id])); // Extract row data
  
//     // Add a page header
//     doc.setFontSize(16);
//     doc.text('Report', 40, 30);
  
//     // Dynamically calculate column widths based on content
//     const columnWidths = columns.map(col => {
//       const maxHeaderWidth = doc.getTextWidth(col.Header);
//       const maxDataWidth = Math.max(...data.map(row => doc.getTextWidth(String(row[col.id]))));
//       return Math.max(maxHeaderWidth, maxDataWidth) + 10; // Add padding
//     });
  
//     // Fit columns to page width
//     const totalWidth = columnWidths.reduce((sum, width) => sum + width, 0);
//     const pageWidth = doc.internal.pageSize.width - 40; // Account for margins
//     const scale = pageWidth / totalWidth; // Calculate scaling factor
  
//     // Adjust column widths to fit page
//     const adjustedColumnWidths = columnWidths.map(width => width * scale);
  
//     // Add a table with adjusted column widths
//     doc.autoTable({
//       head: [tableColumn],
//       body: tableRows,
//       startY: 50,
//       margin: { horizontal: 10 },
//       styles: {
//         cellPadding: 5,
//         fontSize: 8, // Reduce font size to fit more content
//         overflow: 'linebreak',
//         halign: 'center',
//         valign: 'middle'
//       },
//       columnStyles: columns.reduce((styles, col, index) => {
//         styles[col.id] = { cellWidth: adjustedColumnWidths[index] };
//         return styles;
//       }, {}),
//       pageBreak: 'auto', // Allow table to break across pages
//       didDrawPage: (data) => {
//         // Add page number
//         doc.text(`Page ${data.pageNumber}`, doc.internal.pageSize.width - 60, doc.internal.pageSize.height - 30);
//       }
//     });
  
//     doc.save('report.pdf');
//   };
  


//   const downloadExcel = () => {
//     const worksheet = XLSX.utils.json_to_sheet(data);
//     const workbook = XLSX.utils.book_new();
//     XLSX.utils.book_append_sheet(workbook, worksheet, 'Report');
//     const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
//     const dataBlob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
//     saveAs(dataBlob, 'report.xlsx');
//   };

//   const handleSearch = (e) => {
//     const value = e.target.value.toLowerCase();
//     if (value === "") {
//       setData(originalData);
//     } else {
//       setData(originalData.filter(row => {
//         return columns.some(col => String(row[col.id]).toLowerCase().includes(value));
//       }));
//     }
//   };

//   return (
//     <div className="container mt-4">
//       <div className="row col-md-12">
//         <div className="col-md-6">
//           <input
//             type="text"
//             className="form-control mb-3"
//             placeholder="Search..."
//             onChange={handleSearch}
//           />
//         </div>
//         <div className="col-md-6">
//           {role !== '' && role !== "User" && (
//             <div className='d-grid gap-2 d-md-flex justify-content-md-end'>
//               <button className="btn btn-primary me-2" onClick={downloadPdf}>Download PDF</button>
//               <button className="btn btn-success" onClick={downloadExcel}>Download Excel</button>
//             </div>
//           )}
//         </div>
//       </div>
      
//       <div className="scrollbar-wrapper">
//         <div className="table-wrapper">
//           <table className="table table-striped">
//             <thead>
//               <tr>
//                 {columns.map(column => (
//                   <th key={column.id}>{column.Header}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody>
//               {data.map((row, i) => (
//                 <tr key={i}>
//                   {columns.map(column => (
//                     <td key={column.id}>{row[column.id]}</td>
//                   ))}
//                 </tr>
//               ))}
//             </tbody>
//           </table>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default Report;



import React, { useState, useEffect } from 'react';
import jsPDF from 'jspdf';
import 'jspdf-autotable';
import * as XLSX from 'xlsx';
import { saveAs } from 'file-saver';
import 'bootstrap/dist/css/bootstrap.min.css';
import jsonData from './data.json'; // Import the data.json file

const Report = () => {
  const [data, setData] = useState([]); // Displayed data
  const [originalData, setOriginalData] = useState([]); // Original unfiltered data
  const [columns, setColumns] = useState([]);
  const [filters, setFilters] = useState({}); // Track filter values for each column
  const [searchedColumn, setSearchedColumn] = useState(''); // Track user input for column search
  const [selectedColumn, setSelectedColumn] = useState(null); // Track the column to display the filter for

  const role = "Admin"; // Example role, replace with actual role from your auth logic

  useEffect(() => {
    setColumns(jsonData.columns);
    setData(jsonData.data);
    setOriginalData(jsonData.data);

    // Initialize filters state
    const initialFilters = jsonData.columns.reduce((acc, column) => {
      acc[column.id] = '';
      return acc;
    }, {});
    setFilters(initialFilters);
  }, []);

  const downloadPdf = () => {
    const doc = new jsPDF('l', 'pt', 'a4');
    const tableColumn = columns.map(col => col.Header);
    const tableRows = data.map(row => columns.map(col => row[col.id]));

    doc.setFontSize(16);
    doc.text('Report', 40, 30);

    const columnWidths = columns.map(col => {
      const maxHeaderWidth = doc.getTextWidth(col.Header);
      const maxDataWidth = Math.max(...data.map(row => doc.getTextWidth(String(row[col.id]))));
      return Math.max(maxHeaderWidth, maxDataWidth) + 10;
    });

    const totalWidth = columnWidths.reduce((sum, width) => sum + width, 0);
    const pageWidth = doc.internal.pageSize.width - 40;
    const scale = pageWidth / totalWidth;
    const adjustedColumnWidths = columnWidths.map(width => width * scale);

    doc.autoTable({
      head: [tableColumn],
      body: tableRows,
      startY: 50,
      margin: { horizontal: 10 },
      styles: {
        cellPadding: 5,
        fontSize: 8,
        overflow: 'linebreak',
        halign: 'center',
        valign: 'middle'
      },
      columnStyles: columns.reduce((styles, col, index) => {
        styles[col.id] = { cellWidth: adjustedColumnWidths[index] };
        return styles;
      }, {}),
      pageBreak: 'auto',
      didDrawPage: (data) => {
        doc.text(`Page ${data.pageNumber}`, doc.internal.pageSize.width - 60, doc.internal.pageSize.height - 30);
      }
    });

    doc.save('report.pdf');
  };

  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Report');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const dataBlob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(dataBlob, 'report.xlsx');
  };

  const handleFilterChange = (e, columnId) => {
    const value = e.target.value;
    const updatedFilters = { ...filters, [columnId]: value };
    setFilters(updatedFilters);

    const filteredData = originalData.filter(row => {
      return Object.keys(updatedFilters).every(key => {
        if (!updatedFilters[key]) return true;
        return String(row[key]).toLowerCase().includes(updatedFilters[key].toLowerCase());
      });
    });

    setData(filteredData);
  };

  const handleSearchColumn = () => {
    const foundColumn = columns.find(col => col.Header.toLowerCase() === searchedColumn.toLowerCase());
    if (foundColumn) {
      setSelectedColumn(foundColumn);
    } else {
      alert('Field not found. Please enter a valid column name.');
      setSelectedColumn(null); // Clear the selected column if not found
    }
  };

  return (
    <div className="container mt-4">
      

      {role !== '' && role !== "User" && (
        <div className='d-grid gap-2 d-md-flex justify-content-md-end mb-3'>
          <button className="btn btn-primary me-2" onClick={downloadPdf}>Download PDF</button>
          <button className="btn btn-success" onClick={downloadExcel}>Download Excel</button>
        </div>
      )}

<div className="row col-md-8">
        <div className="col-md-8">
          <input
            type="text"
            className="form-control mb-3 shadow-none"
            placeholder="Enter column name..."
            value={searchedColumn}
            onChange={(e) => setSearchedColumn(e.target.value)}
          />
        </div>
        <div className="col-md-4 ">
          <button className="btn btn-primary " onClick={handleSearchColumn}>
            Search Column
          </button>
        </div>
      </div>

      {selectedColumn && (
        <div className="row col-md-12 mb-3">
          <div className="col-md-3">
            <label>{selectedColumn.Header}</label>
            <select
              className="form-select"
              value={filters[selectedColumn.id]}
              onChange={(e) => handleFilterChange(e, selectedColumn.id)}
            >
              <option value="">All</option>
              {[...new Set(originalData.map(row => row[selectedColumn.id]))].map((option, index) => (
                <option key={index} value={option}>{option}</option>
              ))}
            </select>
          </div>
        </div>
      )}

      <div className="scrollbar-wrapper">
        <div className="table-wrapper">
          <table className="table table-striped">
            <thead>
              <tr>
                {columns.map(column => (
                  <th key={column.id}>{column.Header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.map((row, i) => (
                <tr key={i}>
                  {columns.map(column => (
                    <td key={column.id}>{row[column.id]}</td>
                  ))}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
};

export default Report;
